...
test("parses doctor, dry-run, and json", () => {
  assert.deepEqual(parseCliArgs(["doctor", "--dry-run", "--json"]), {
    command: "doctor",
    server: false,
    port: 0,
    open: false,
...
test("open implies server with doc-gen input", () => {
  const parsed = parseCliArgs(["--doc-gen", "legacy/api-docs.db", "--open"]);
...
test("defaults doc-gen and out paths under .lean-view", () => {
  ...
  assert.equal(resolved.rootModule, "CurrentProject");
...
test("does not generate doc-gen when doc-gen is explicitly provided", () => {
  assert.equal(resolveCliOptions(parseCliArgs(["--doc-gen", "docbuild"]), "/repo").generateDocGen, false);
});

test("derives repo root, root module, and project name from ancestor lakefile.toml", () => {
  withTempDir((root) => {
    ...
    const resolved = resolveCliOptions(parseCliArgs(["--root-module", "Fallback", "--project-name", "Fallback Name"]), nested);

    assert.equal(resolved.repoRoot, project);
    assert.equal(resolved.rootModule, "MockProject");
...
test("derives project metadata from lakefile.lean", () => {
  assert.deepEqual(
    ...
    { projectName: "mock-service", rootModule: "MockService" },
  );
});

test("uses root module and project name options as fallback when lakefile metadata is unavailable", () => {
  withTempDir((root) => {
    writeFileSync(join(root, "lakefile.toml"), 'version = "0.1.0"\n', "utf8");

    const resolved = resolveCliOptions(
      parseCliArgs(["--root-module", "FallbackRoot", "--project-name", "Fallback Name"]),
      root,
    );

    assert.equal(resolved.rootModule, "FallbackRoot");
    assert.equal(resolved.projectName, "Fallback Name");
    assert.equal(resolved.packageName, basename(root));
  });
});

test("falls back to the Lake directory name for root module when options are absent", () => {
  withTempDir((root) => {
